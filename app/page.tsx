    ...
    <main className="min-h-screen bg-slate-900 bg-[url('/grid.svg')] bg-center [mask-image:linear-gradient(180deg,white,rgba(255,255,255,0))]">
        ...
        <div className="flex items-center justify-between px-6 h-16 max-w-7xl mx-auto">
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono font-bold tracking-tighter text-lg text-white">
              NDAA<span className="text-cyber">.DIR</span>
            </span>
          </div>
          <nav className="flex items-center gap-6 text-sm font-medium text-slate-400">
            <a href="#" className="hover:text-cyber transition-colors">Directory</a>
            <a href="#" className="hover:text-cyber transition-colors">Compliance</a>
            <a href="#" className="hover:text-cyber transition-colors">Manufacturers</a>
            <button className="px-4 py-2 text-xs font-bold text-slate-900 bg-cyber rounded hover:bg-sky-400 transition-colors">
              AGENCY LOGIN
            </button>
          </nav>
        </div>
      ...
      <div className="relative pt-32 pb-20 px-6 max-w-7xl mx-auto">
        <div className="flex flex-col items-center text-center space-y-8">
          ...
          <h1 className="text-5xl md:text-7xl font-bold tracking-tight text-white max-w-4xl">
            Secure Robotics <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyber to-emerald-400">
              Procurement Nexus
            </span>
          </h1>

          <p className="text-slate-400 text-lg max-w-2xl leading-relaxed">
            The definitive registry for NDAA-compliant unmanned systems.
            Verified hardware for government and defense applications.
          </p>

          {/* Search Bar */}
          <div className="w-full max-w-2xl relative group">
            <div className="absolute inset-0 bg-gradient-to-r from-cyber via-emerald-500 to-cyber opacity-20 blur-xl group-hover:opacity-30 transition-opacity rounded-lg"></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 px-6 max-w-7xl mx-auto mb-20">
        {[
          ...
          { label: "Active Agencies", value: "312", icon: Terminal, color: "text-cyber" },
        ].map((stat, i) => (
      ...
      {/* Recent Listings (Mock) */}
      <div className="px-6 max-w-7xl mx-auto pb-20">
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-xl font-bold text-white flex items-center gap-2">
            <span className="w-1 h-6 bg-cyber rounded-full"></span>
            Recent Certifications
          </h2>
          <button className="text-sm text-cyber hover:text-white flex items-center gap-1 transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map((item) => (
            <div key={item} className="group relative bg-slate-900 border border-slate-800 hover:border-cyber/50 rounded-xl overflow-hidden transition-all duration-300 hover:shadow-[0_0_20px_rgba(56,189,248,0.1)]">
              {/* Image Placeholder */}
              ...
              {/* Content */}
              <div className="p-4">
                <h3 className="text-lg font-bold text-white group-hover:text-cyber transition-colors">Skydio X10</h3>
